...
#define kDefaultMaxLimitedDate @"2024-12-31 23:59"
...
#define kRowNormalStatusColor [UIColor colorWithRed:60/255.0 green:210/255.0 blue:210/255.0 alpha:1]
#define kWidthOfTotal self.frame.size.width
#define kHeightOfButtonContentView 45.0
#define kButtonNormalStatusColor [UIColor colorWithRed:60/255.0 green:210/255.0 blue:210/255.0 alpha:1]
...
- (void)addUnitLabel:(NSString *)text withPointX:(CGFloat)pointX {
    ...
    lblUnit.textColor = kRowNormalStatusColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    // 初始化头部按钮（取消、现在时间、确定）
    self.buttonContentView = [[UIView alloc] initWithFrame:CGRectMake(-2.0, 0.0, kWidthOfTotal + 4.0, kHeightOfButtonContentView)];
    self.buttonContentView.backgroundColor = rgb(216, 216, 216, 1);
    //    self.buttonContentView.layer.borderColor = [UIColor lightGrayColor].CGColor;
    //    self.buttonContentView.layer.borderWidth = 0.5;
    [self addSubview:self.buttonContentView];
    
    self.btnCancel = [UIButton buttonWithType:UIButtonTypeCustom];
    self.btnCancel.frame = CGRectMake(2.0, 2.5, 60.0, kHeightOfButtonContentView - 5.0);
    self.btnCancel.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    [self.btnCancel setTitle:INTER_Str(@"Cancel") forState:UIControlStateNormal];
    [self.btnCancel setTitleColor:kButtonNormalStatusColor forState:UIControlStateNormal];
    [self.btnCancel addTarget:self
                       action:@selector(cancel:)
             forControlEvents:UIControlEventTouchUpInside];
    [self.buttonContentView addSubview:self.btnCancel];
    
    if ([self canShowScrollToNowButton]) {
        self.btnScrollToNow = [UIButton buttonWithType:UIButtonTypeCustom];
        self.btnScrollToNow.frame = CGRectMake(self.buttonContentView.frame.size.width/2 - 50.0, 2.5, 100.0, kHeightOfButtonContentView - 5.0);
        self.btnScrollToNow.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
        [self.btnScrollToNow setTitle:INTER_Str(@"NowTime") forState:UIControlStateNormal];
        [self.btnScrollToNow setTitleColor:kButtonNormalStatusColor forState:UIControlStateNormal];
        [self.btnScrollToNow addTarget:self
                                action:@selector(scrollToNowDateIndexPosition:)
                      forControlEvents:UIControlEventTouchUpInside];
        //        [self.buttonContentView addSubview:self.btnScrollToNow];
    }
    
    self.btnConfirm = [UIButton buttonWithType:UIButtonTypeCustom];
    self.btnConfirm.frame = CGRectMake(kWidthOfTotal - 58.0, 2.5, 60.0, kHeightOfButtonContentView - 5.0);
    self.btnConfirm.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    [self.btnConfirm setTitle:INTER_Str(@"OK") forState:UIControlStateNormal];
    [self.btnConfirm setTitleColor:kButtonNormalStatusColor forState:UIControlStateNormal];
    [self.btnConfirm addTarget:self
                        action:@selector(confirm:)
              forControlEvents:UIControlEventTouchUpInside];
    [self.buttonContentView addSubview:self.btnConfirm];
    ...
    switch (_datePickerStyle) {
        case KMDatePickerStyleYearMonthDayHourMinute: {
            ...
            switch (component) {
                case 0:
                    ...
                    [self addUnitLabel:INTER_Str(@"Year") withPointX:width/2 + 24.0];
                    break;
                case 1:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Month") withPointX:(widthOfAverage + 20.0) + width/2 + 16.0];
                    break;
                case 2:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Day") withPointX:(2*widthOfAverage + 20.0) + width/2 + 22.0];
                    break;
                case 3:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Hour") withPointX:(3*widthOfAverage + 20.0) + width/2 + 28.0];
                    break;
                case 4:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Minute") withPointX:(4*widthOfAverage + 20.0) + width/2 + 32.0];
                    break;
            }
            break;
        }
        ...
        case KMDatePickerStyleMonthDayHourMinute: {
            ...
            switch (component) {
                case 0:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Month") withPointX:width/2 + 11.0];
                    break;
                case 1:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Day") withPointX:widthOfAverage + width/2 + 17.0];
                    break;
                case 2:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Hour") withPointX:2*widthOfAverage + width/2 + 23.0];
                    break;
                case 3:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Minute") withPointX:3*widthOfAverage + width/2 + 27.0];
                    break;
            }
            break;
        }
        case KMDatePickerStyleHourMinute: {
            ...
            switch (component) {
                case 0:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Hour") withPointX:width/2 + 12.0];
                    break;
                case 1:
                    width = widthOfAverage;
                    [self addUnitLabel:INTER_Str(@"Minute") withPointX:widthOfAverage + width/2 + 18.0];
                    break;
            }
            break;
        }
